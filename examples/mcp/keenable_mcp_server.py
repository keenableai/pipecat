...
Reads ``KEENABLE_API_KEY`` from the environment and speaks MCP over stdio
so any MCP client (including ``pipecat.services.mcp_service.MCPClient``
with ``StdioServerParameters``) can connect.

Run directly for debugging::

    KEENABLE_API_KEY=... python examples/mcp/keenable_mcp_server.py
"""
...
async def keenable_search(query: str) -> dict[str, Any]:
    ...
    results = await _get_client().search(query)
...
async def keenable_fetch(url: str) -> dict[str, Any]:
    ...
    return (await _get_client().fetch(url)).model_dump()


if __name__ == "__main__":
    if not os.getenv("KEENABLE_API_KEY"):
        print("KEENABLE_API_KEY is not set", file=sys.stderr)
        sys.exit(1)
    mcp.run("stdio")
